...
- describe and extension of the implementation to calculate distances of mini-batches
...
Just as we calculated. Now, it would be very very interesting to check the matrices returned by the `sinkhorn()` method: `P`, the calculated coupling matrix, and `C`, the distance matrix. Let's begin with the distance matrix:
...
<ol>
    ...
    <li>Charlie Frogner, Farzaneh Mirzazadeh, Justin Solomon. "Learning Embeddings into Entropic Wasserstein Spaces." ICLR (2019).
</ol>
